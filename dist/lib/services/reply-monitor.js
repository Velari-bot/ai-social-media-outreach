...
exports.monitorAllReplies = monitorAllReplies;
...
async function getAccountName(auth) {
    try {
...
async function monitorAllReplies() {
    console.log('[Reply Monitor] Starting reply check for all users...');
    // Get all users with Gmail connected and AI enabled
    const settingsSnapshot = await firebase_admin_1.db.collection('user_email_settings')
    ...
    for (const settingsDoc of settingsSnapshot.docs) {
        ...
        try {
            ...
            await firebase_admin_1.db.collection('user_email_settings').doc(userId).update({
                last_reply_check: firestore_1.Timestamp.now(),
...
async function checkUserReplies(userId) {
        ...
        try {
            ...
            for (const thread of threads) {
                try {
                    ...
                    let threadDoc = await firebase_admin_1.db.collection('email_threads').doc(thread.id).get();
                    ...
                    if (!threadDoc.exists) {
                        ...
                        let queueSnap = await firebase_admin_1.db.collection('outreach_queue')
                            .where('gmail_thread_id', '==', thread.id)
                            .limit(1)
                            .get();
                        // 2. Fallback to creator email
                        if (queueSnap.empty) {
                            queueSnap = await firebase_admin_1.db.collection('outreach_queue')
                        ...
                        if (!queueSnap.empty) {
                            ...
                            await firebase_admin_1.db.collection('email_threads').doc(thread.id).set({
                                user_id: userId,
                            ...
                            const queueDocs = await firebase_admin_1.db.collection('outreach_queue')
                                .where('creator_email', '==', creatorEmail)
                                .where('user_id', '==', userId)
                                .limit(1)
                                .get();
                            ...
                            threadDoc = await firebase_admin_1.db.collection('email_threads').doc(thread.id).get();
                    ...
                    if (aiResponse.toLowerCase().includes('ignore')) {
                        ...
                        await firebase_admin_1.db.collection('email_threads').doc(thread.id).update({
                            status: 'closed',
                            updated_at: firestore_1.Timestamp.now()
                        });
                    ...
                    await firebase_admin_1.db.collection('user_email_settings').doc(userId).set({
                        total_replies_received: firestore_1.FieldValue.increment(1),
                        updated_at: firestore_1.Timestamp.now()
                    }, { merge: true });
                    // Mark read
                    await gmail.users.threads.modify({
...
async function extractCreatorData(message) {
    const completion = await getOpenAI().chat.completions.create({
        model: "gpt-4",
        messages: [
            {
...
async function sendGmailReply(gmail, params) {
